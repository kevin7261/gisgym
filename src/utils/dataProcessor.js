...
export async function loadGeoJsonForRoutes(layer) {
  try {
    ...
    // 提取路線數據（只包含 type === "way" 的 LineString/MultiLineString）
    ...
    // 根據 route_id 或 route_name 分組路線（避免重複）
    const routeMap = new Map();
    routeFeatures.forEach((feature) => {
      const tags = feature.properties.tags || {};
      const routeId = tags.route_id || tags.route_name || `route_${feature.properties.id}`;
    ...
    return {
      jsonData: null, // GeoJSON 數據不再存儲在 jsonData 中
      processedJsonData: geojson, // 處理後的數據（這裡就是原始 GeoJSON）
      geojsonData: geojson, // GeoJSON 數據存儲在 geojsonData 中
      dashboardData,
      dataTableData, // 只包含路線的表格數據
      layerInfoData,
    };
  } catch (error) {
...
export async function loadSpaceNetworkGridJson(layer) {
  try {
    ...
    return {
      jsonData: jsonData, // 保持原始數據不變
      spaceNetworkGridJsonData: JSON.parse(JSON.stringify(routesData)), // 空間網絡網格數據（深拷貝）
      layoutGridJsonData: JSON.parse(JSON.stringify(routesData)), // 版面網格數據（深拷貝）
      layoutGridJsonData_Test: JSON.parse(JSON.stringify(routesData)), // 版面網格測試數據（深拷貝）
      layoutGridJsonData_Test2: JSON.parse(JSON.stringify(routesData)), // 版面網格測試2數據（深拷貝）
      processedJsonData: routesData, // 處理後的數據（routesData 也賦值給 processedJsonData）
      dashboardData,
      dataTableData,
      layerInfoData,
    };
 ...
 * 1. 只賦值給 layoutGridJsonData_Test3
 * 2. 所有座標 *2（確保座標一定是偶數）
 *
 * @param {Object} layer - 圖層配置對象
 * @returns {Promise<Object>} 包含處理後數據的對象
