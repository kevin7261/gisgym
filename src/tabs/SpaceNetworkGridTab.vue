<script setup>
  ...
  const drawMap = () => {
    ...
    // 根據範圍大小決定網格間距（大約10-20個網格）
    const xGridStep = Math.max(1, Math.ceil(xRange / 15));
    const yGridStep = Math.max(1, Math.ceil(yRange / 15));
  ...
  /**
   * 👀 監聽當前圖層的主要示意圖資料變化
   * 當圖層的 spaceNetworkGridJsonData 載入完成時，自動載入並繪製
   */
  watch(
    () => {
      if (!activeLayerTab.value) return null;
