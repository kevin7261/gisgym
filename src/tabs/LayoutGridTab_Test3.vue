<script setup>
  ...
  const drawMap = () => {
    ...
    const targetLayerForMeta = activeLayerTab.value ? dataStore.findLayerById(activeLayerTab.value) : null;
    ...
    const totalColMaxValue = Object.keys(colMaxValues)
      .map((x) => Number(x))
      .filter((x) => Number.isFinite(x) && !fixedColSet.has(x)) // 排除固定為 1pt 的列
      .reduce((sum, x) => sum + (colMaxValues[x] || 0), 0);
    const totalRowMaxValue = Object.keys(rowMaxValues)
      .map((y) => Number(y))
      .filter((y) => Number.isFinite(y) && !fixedRowSet.has(y)) // 排除固定為 1pt 的行
      .reduce((sum, y) => sum + (rowMaxValues[y] || 0), 0);
    ...
    if (isTest2EdgeMode) {
      if (targetLayer?.dataTableData && Array.isArray(targetLayer.dataTableData)) {
        console.log('📊 處理 edgeMaxValues（dataTableData），項目數:', targetLayer.dataTableData.length);
