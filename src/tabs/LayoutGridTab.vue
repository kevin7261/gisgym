<script setup>
  ...
  const drawMap = () => {
    ...
    // 如果寬度或高度大於 100，使用和 SpaceNetworkGridTab 一樣的刻度設置
    // 否則每個刻度都繪製網格線
    const xGridStep = xRange > 100 ? Math.max(1, Math.ceil(xRange / 15)) : 1;
    const yGridStep = yRange > 100 ? Math.max(1, Math.ceil(yRange / 15)) : 1;
